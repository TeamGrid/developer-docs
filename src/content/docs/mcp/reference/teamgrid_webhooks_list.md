---
title: teamgrid_webhooks_list
description: "List configured TeamGrid webhooks without modifying them."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_webhooks_list` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`listWebhooks`](/api/v1/reference/operations/listwebhooks/) — `GET /webhooks`
...
- Webhook read responses do not contain the reveal-once signing secret.
...
> List the first 20 TeamGrid webhooks. Do not request another page; tell me whether another cursor is available.
...
| The credential lacks `webhooks:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
