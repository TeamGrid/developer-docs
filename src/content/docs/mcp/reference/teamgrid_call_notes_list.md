---
title: teamgrid_call_notes_list
description: "List TeamGrid call notes. Results can contain sensitive conversation data."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_call_notes_list` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
Required scope: `call-notes:read`.

- [`listCallNotes`](/api/v1/reference/operations/listcallnotes/) — `GET /call-notes`
...
**conversation-data:** The response can contain sensitive plain-text conversation data.
...
> List the first 20 TeamGrid call notes. Do not request another page; tell me whether another cursor is available.
...
| The credential lacks `call-notes:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
