---
title: teamgrid_contact_groups_list
description: "List TeamGrid contact groups with stable cursor pagination."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_contact_groups_list` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
Required scope: `contact-groups:read`.

- [`listContactGroups`](/api/v1/reference/operations/listcontactgroups/) — `GET /contact-groups`
...
> List the first 20 TeamGrid contact groups. Do not request another page; tell me whether another cursor is available.
...
| The credential lacks `contact-groups:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
