...
The stable `1.1.0` manifest records 153 v1 paths, 236 governed v1 operations, 87 frozen v0
...
reads, and another 46 domain-specific `If-Match` operations. These counts describe the synchronized
...
release they tested. The `1.1.0` checkpoint qualifies core project, task, and project-template
CAS while preserving all domain-specific preconditions. Its synchronized SDK, CLI, and MCP package
checkpoint is `1.1.0`. See [resource concurrency](/api/v1/resource-concurrency/).
