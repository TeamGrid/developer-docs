...
Required scope: `product-groups:read`.

- [`getProductGroup`](/api/v1/reference/operations/getproductgroup/) — `GET /product-groups/{id}`
...
> Read the TeamGrid product group with ID `<id>` and summarize only the fields returned by TeamGrid.
