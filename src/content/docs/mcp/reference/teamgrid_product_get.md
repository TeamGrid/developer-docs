---
title: teamgrid_product_get
description: "Get one TeamGrid product without exposing its purchase price."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_product_get` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`getProduct`](/api/v1/reference/operations/getproduct/) — `GET /products/{id}`
...
- `purchasePrice` is removed even when the credential has a finance overlay.
...
> Read the TeamGrid product with ID `<id>` and summarize only the fields returned by TeamGrid.
...
| The credential lacks `products:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
