...
| --- | ---: | --- |
| Released in the release-candidate contract | 48 | A bounded public v1 workflow is implemented across its required surfaces |
| Partial | 8 | Some useful behavior exists, but the product workflow is not yet complete |
...
project reads, task reads, writes and lifecycle, complete non-billing time-entry reads and writes,
complete contact reads, and webhook-secret rotation are now released in the
...
classification applies to discovery, credentials, custom-field values, project templates,
planned-work lifecycle, audit, and
