---
title: teamgrid_time_entries_list
description: "List TeamGrid time entries with stable cursor pagination."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_time_entries_list` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`listTimeEntries`](/api/v1/reference/operations/listtimeentries/) — `GET /time-entries`
...
**work-record-data:** Time-entry records can expose individual work activity and billing state.
...
> List the first 20 TeamGrid time entries. Do not request another page; tell me whether another cursor is available.
...
| The credential lacks `time-entries:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
