---
title: teamgrid_tasks_list
description: "List TeamGrid tasks with optional project, assignee, and status filters."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_tasks_list` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`listTasks`](/api/v1/reference/operations/listtasks/) — `GET /tasks`
...
> List the first 20 TeamGrid tasks. Do not request another page; tell me whether another cursor is available.
...
| The credential lacks `tasks:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
