---
title: teamgrid_tag_get
description: "Get one TeamGrid tag by id."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_tag_get` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`getTag`](/api/v1/reference/operations/gettag/) — `GET /tags/{id}`
...
> Read the TeamGrid tag with ID `<id>` and summarize only the fields returned by TeamGrid.
...
| The credential lacks `tags:read` or cannot access the requested resource. | The tool returns `teamgrid_request_failed`; the redacted detail comes from the rejected TeamGrid request. |
