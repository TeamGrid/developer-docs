---
title: teamgrid_task_recurrence_get
description: "Get one recurring TeamGrid task definition, including its current immutable policy version."
owner: Developer Platform
reviewedAt: 2026-08-10
---

`teamgrid_task_recurrence_get` is a read-only, idempotent TeamGrid MCP tool. It is introduced by the
...
- [`getTaskRecurrence`](/api/v1/reference/operations/gettaskrecurrence/) — `GET /task-recurrences/{id}`
...
> Read the TeamGrid task recurrence with ID `<id>` and summarize only the fields returned by TeamGrid.
